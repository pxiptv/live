...
try:
    with open('whitelist.txt', 'w', encoding='utf-8') as output_file:
        for line in lines:
            ...
            if "[240" in line:
                # 如果行中包含 "[240"，使用 ping6 进行检测
                try:
                    result = subprocess.run(['ping6', '-c', '4', url], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                    if result.returncode == 0:
                        print(f'ping6检测正常: {name},{url}')
                        output_file.write(line)
                    else:
                        print(f'ping6检测失败: {name},{url}')
                        append_to_file('blacklist.txt', line)
                except Exception as e:
                    print(f'ping6检测错误: {name},{url}, Error: {e}')
                    append_to_file('blacklist.txt', line)
            else:
                try:
                    if "://" in url:
